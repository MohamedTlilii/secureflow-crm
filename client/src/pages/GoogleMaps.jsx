...
import { Plus, MapPin, Phone, User, X, Edit2, Trash2, MessageSquare, Bell, Sparkles, AlertTriangle, Building2 } from 'lucide-react';
...
export default function GoogleAlerts() {
  ...
  const ini = p => ((p.entreprise?.[0] || p.prenom?.[0] || 'G')).toUpperCase();
  const displayName = p => p.entreprise || (p.prenom || p.nom ? `${p.prenom} ${p.nom}`.trim() : 'Alerte Google');
  ...
  const handleAnalyze = async () => {
    ...
    setAiLoading(true);
    try {
      const res = await axios.post('/api/google-alerts/analyze-gemini', { alertText: alertInput });
      if (res.data.exclure) {
        toast('Alerte exclue — ' + res.data.raison, { icon: '🚫', duration: 4000 });
      } else {
      ...
      if (err.response?.status === 429) {
        toast.error('Limite atteinte — réessaie dans 1 minute', { duration: 5000 });
      } else {
        toast.error('Erreur — ' + (err.response?.data?.message || 'vérifie le backend'));
      }
    } finally {
      setAiLoading(false);
    }
    ...
    <div className="animate-fade">
      <div className="page-header flex-between">
        <div style={{display:'flex',alignItems:'center',gap:12}}>
          ...
          <div>
            <h1>Google Alerts</h1>
            <p style={{color:'var(--text-muted)',fontSize:13}}>Alertes analysées par Groq AI — gratuit</p>
          </div>
        </div>
              ...
              <div key={p._id} className="card" onClick={()=>openDetail(p)}
                ...
                <div style={{display:'flex',alignItems:'center',gap:12,marginBottom:12}}>
                  ...
                  <div style={{minWidth:0}}>
                    <div style={{fontWeight:600,fontSize:15,whiteSpace:'nowrap',overflow:'hidden',textOverflow:'ellipsis'}}>{displayName(p)}</div>
                    {p.entreprise && (p.prenom||p.nom) && (
                      <div style={{fontSize:12,color:'var(--text-muted)',whiteSpace:'nowrap',overflow:'hidden',textOverflow:'ellipsis'}}>{p.prenom} {p.nom}</div>
                    )}
                  </div>
                </div>
                ...
                <div style={{display:'flex',gap:6,flexWrap:'wrap',marginBottom:12}}>
                  ...
                  <span style={{fontSize:10,fontWeight:600,padding:'2px 8px',borderRadius:20,background:'#ea433515',color:'#ea4335'}}>🔔 Google Alert</span>
                  {p.urgencyScore > 0 && (
                ...
                <div style={{display:'flex',flexDirection:'column',gap:5,marginBottom:12}}>
                  {p.entreprise && <span style={{fontSize:12,color:'var(--text-secondary)',display:'flex',alignItems:'center',gap:6}}><Building2 size={11}/>{p.entreprise}</span>}
                ...
                {/* Résumé IA */}
                {p.aiSummary && (
                  <div style={{fontSize:12,color:'var(--text-secondary)',marginBottom:12,lineHeight:1.5,background:'var(--bg-secondary)',padding:'8px 10px',borderRadius:8,borderLeft:'3px solid #ea4335',display:'-webkit-box',WebkitLineClamp:2,WebkitBoxOrient:'vertical',overflow:'hidden'}}>
                    {p.aiSummary}
                  </div>
                )}
        ...
        <div className="modal-overlay" onClick={e=>{if(e.target===e.currentTarget)setModal(null)}}>
          <div className="modal">
            ...
            <p style={{fontSize:13,color:'var(--text-muted)',marginBottom:16}}>
              Colle le texte de ton email Google Alert — Groq extrait les infos et crée la fiche automatiquement.
            </p>
            <textarea
              className="input"
          ...
          <div className="modal">
            <div className="modal-header">
              <div style={{display:'flex',alignItems:'center',gap:12}}>
                ...
                <div>
                  <h2 style={{margin:0}}>{displayName(selected)}</h2>
            ...
            <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:10,marginBottom:16}}>
              {[
                ...
                ['Mot-clé', selected.keyword||'—'],
                ['Adresse', selected.adresse||'—'],
                ['Ajouté le', new Date(selected.createdAt).toLocaleDateString('fr-CA')]
              ].map(([label,val])=>(
            ...
            {selected.aiSummary && (
              <div style={{marginBottom:12}}>
                <div style={{fontSize:11,color:'var(--text-muted)',fontWeight:600,textTransform:'uppercase',marginBottom:6,display:'flex',alignItems:'center',gap:6}}><Sparkles size={11} color="#ea4335"/>Résumé Groq</div>
            ...
            {selected.alertText && (
              <div style={{marginBottom:12}}>
                <div style={{fontSize:11,color:'var(--text-muted)',fontWeight:600,textTransform:'uppercase',marginBottom:6}}>Texte original</div>
                <div style={{background:'var(--bg-secondary)',borderRadius:8,padding:'10px 12px',fontSize:12,color:'var(--text-secondary)',lineHeight:1.6,maxHeight:120,overflowY:'auto'}}>{selected.alertText}</div>
              </div>
            )}
            <div className="modal-footer">
      ...
      {modal==='notes' && selected && (
        <div className="modal-overlay" onClick={e=>{if(e.target===e.currentTarget)setModal(null)}}>
          <div className="modal">
            <div className="modal-header">
              <h2>Notes — {displayName(selected)}</h2>
              <button className="btn btn-ghost btn-sm" onClick={()=>setModal(null)}><X size={16}/></button>
            </div>
