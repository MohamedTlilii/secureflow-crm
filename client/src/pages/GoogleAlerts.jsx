...
import { Search, Plus, ExternalLink, AlertCircle, Newspaper, X } from 'lucide-react';
import toast from 'react-hot-toast';
import axios from 'axios';

const EMPTY_PROSPECT = { prenom:'', nom:'', entreprise:'', type:'B2B', ville:'Montreal', telephone:'', email:'', linkedin:'', priorite:'P1', stage:'01_Inbox', signal:'manuel', valeurEstimee:0, source:'manuel' };
function parseAlerts(raw) {
  const cards = [];
  // Sépare par "Signaler comme non pertinent" ou lignes vides multiples
  const blocks = raw.split(/signaler comme non pertinent/gi).map(b => b.trim()).filter(Boolean);

  blocks.forEach((block, i) => {
    const lines = block.split('\n').map(l => l.trim()).filter(Boolean);
    if (lines.length < 2) return;

    // Titre = première ligne non vide de longueur raisonnable
    let title = '';
    let source = '';
    let summary = '';

    // Cherche le titre (ligne la plus longue avant la source)
    for (let j = 0; j < lines.length; j++) {
      const line = lines[j];
      if (!title && line.length > 10 && !line.toLowerCase().startsWith('http')) {
        title = line;
      } else if (title && !source && line.length < 60 && !line.toLowerCase().startsWith('http')) {
        source = line;
      } else if (title && source && !summary && line.length > 20) {
        summary = line;
        break;
      }
    }
    ...
    // Détecte la ville
    const villeMatch = raw.match(/Montréal|Laval|Longueuil|Québec|Gatineau|Boucherville|Terrebonne|Repentigny/i);
    const ville = villeMatch ? villeMatch[0] : 'Montreal';

    // Détecte le signal
    let signal = 'manuel';
    const blockLower = block.toLowerCase();
    if (blockLower.includes('ouverture') || blockLower.includes('lancement') || blockLower.includes('création') || blockLower.includes('nouveau')) signal = 'ouverture';
    else if (blockLower.includes('recrutement') || blockLower.includes('emploi') || blockLower.includes('embauche')) signal = 'recrutement';
    else if (blockLower.includes('expansion') || blockLower.includes('agrandit') || blockLower.includes('deuxième')) signal = 'expansion';

    cards.push({ id: i, title, source: source || 'Source inconnue', summary: summary || '', ville, signal, raw: block });
  });

  return cards;
}

const SIGNAL_LABELS = { ouverture:'Nouveau local', recrutement:'Recrutement', 'nouveau-poste':'Nouveau poste', expansion:'Expansion', commentaire:'Commentaire post', incident:'Incident', manuel:'Manuel' };
const SIGNAL_COLORS = { ouverture:'#12b76a', recrutement:'#3b6cf8', expansion:'#f79009', manuel:'#8b8b9e', incident:'#f04438' };
...
export default function GoogleAlerts() {
  ...
  const analyze = () => {
    ...
    if (result.length === 0) { toast.error('Aucun résultat trouvé — vérifie le format'); return; }
    setCards(result);
    setAnalyzed(true);
    toast.success(`${result.length} alerte${result.length > 1 ? 's' : ''} trouvée${result.length > 1 ? 's' : ''} !`);
  };

  const openAdd = (card) => {
    // Pre-rempli avec les infos de la card
setForm({ ...EMPTY_PROSPECT, entreprise: card.source, ville: card.ville, signal: card.signal, source: 'google_alert' });
    setAddModal(card);
  };
    ...
    <div className="animate-fade">
      ...
      <div className="page-header flex-between">
        <div style={{ display:'flex', alignItems:'center', gap:12 }}>
          {/* Google G icon */}
          <div style={{ width:40, height:40, borderRadius:10, background:'white', display:'flex', alignItems:'center', justifyContent:'center', boxShadow:'0 2px 8px rgba(0,0,0,0.15)', flexShrink:0 }}>
          ...
          <div>
            ...
            <p style={{ margin:0, fontSize:12, color:'var(--text-muted)' }}>Transforme tes alertes en prospects</p>
          </div>
        </div>
      ...
      <div className="card" style={{ padding:20, marginBottom:20 }}>
        <div style={{ fontSize:13, fontWeight:600, color:'var(--text-primary)', marginBottom:8 }}>
          📋 Colle ton email Google Alert ici
        </div>
        <div style={{ fontSize:12, color:'var(--text-muted)', marginBottom:12 }}>
          Ouvre Gmail → copie le contenu de l'alerte → colle ici → clique Analyser
        </div>
        <textarea
          value={raw}
          onChange={e => { setRaw(e.target.value); setAnalyzed(false); setCards([]); }}
          placeholder="(mécanique OR pneus...) ACTUALITÉS&#10;Genesis GV60 Magma : dévoilé en primeur à Montréal...&#10;RPM&#10;Genesis choisit Montréal pour présenter...&#10;Signaler comme non pertinent&#10;..."
        ...
        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginTop:12 }}>
          {raw && <button onClick={() => { setRaw(''); setCards([]); setAnalyzed(false); }} style={{ fontSize:12, color:'var(--danger)', background:'none', border:'none', cursor:'pointer' }}>Effacer</button>}
          <div style={{ flex:1 }} />
          <button className="btn btn-primary" onClick={analyze} style={{ gap:8 }}>
        ...
        <>
          ...
          {cards.length === 0 ? (
            <div className="empty-state">
              <AlertCircle size={40} />
              <p>Aucun résultat — vérifie le format de l'email</p>
            </div>
          ) : (
            <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill, minmax(300px, 1fr))', gap:16 }}>
              {cards.map((card, i) => (
                <div key={card.id} className="card"
                  style={{ padding:20, transition:'transform 0.15s, box-shadow 0.15s', cursor:'default' }}
                  ...
                  <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:10 }}>
                    ...
                    <span style={{ fontSize:10, fontWeight:600, padding:'2px 8px', borderRadius:20, background:`${SIGNAL_COLORS[card.signal]}20`, color:SIGNAL_COLORS[card.signal] }}>
                  ...
                  {/* Résumé */}
                  {card.summary && (
                    <div style={{ fontSize:12, color:'var(--text-secondary)', marginBottom:12, lineHeight:1.5, display:'-webkit-box', WebkitLineClamp:3, WebkitBoxOrient:'vertical', overflow:'hidden' }}>
                      {card.summary}
                    </div>
                  )}
                  ...
                  <div style={{ display:'flex', gap:8, borderTop:'1px solid var(--border)', paddingTop:12 }}>
                    <button className="btn btn-primary btn-sm" style={{ flex:1, justifyContent:'center' }} onClick={() => openAdd(card)}>
                      <Plus size={12} /> Ajouter prospect
                    </button>
                  </div>
                </div>
              ))}
        ...
        <div className="modal-overlay" onClick={e => { if (e.target === e.currentTarget) setAddModal(null); }}>
          <div className="modal">
            ...
            <div style={{ fontSize:12, color:'var(--text-muted)', marginBottom:16, background:'var(--bg-secondary)', padding:'8px 12px', borderRadius:8 }}>
              Source : {addModal.source} · {addModal.title.substring(0, 60)}...
            </div>
