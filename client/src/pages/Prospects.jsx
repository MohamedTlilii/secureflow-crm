...
export default function Prospects() {
  ...
  const fetchProspects = useCallback(async () => {
    ...
    const r = await axios.get('/api/prospects', { params });
    setProspects(r.data);
    setLoading(false);
  }, [search, filters]);
      ...
      {modal==='notes' && selected && (
        <div className="modal-overlay" onClick={e=>{if(e.target===e.currentTarget)setModal(null)}}>
          <div className="modal">
            ...
            <div style={{maxHeight:250,overflowY:'auto',display:'flex',flexDirection:'column',gap:8,marginBottom:16}}>
              {selected.notes?.length ? selected.notes.map((n,i)=>(
                <div key={i} style={{background:'var(--bg-secondary)',borderRadius:8,padding:'10px 12px'}}>
                  ...
                  <div style={{fontSize:11,color:'var(--text-muted)'}}>{n.author} · {new Date(n.date).toLocaleDateString('fr-CA')}</div>
                </div>
